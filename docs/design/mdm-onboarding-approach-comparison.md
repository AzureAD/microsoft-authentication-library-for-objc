...
2. **Terminal completion URL handling (uniform outcome handling)**
   ...
   - Explicit rule:
     - `msauth://in_app_enrollement_complete` is **NOT intercepted for onboarding work at navigation-time**; it propagates to response-object parsing for uniform outcome handling.
...
- `msauth://enroll` and `msauth://compliance` are intercepted at navigation-time (`NavAction`) with: **Cancel → BRT guard (once per redirect instruction) → build nextRequest → load in same `WKWebView`**.
