...
git -c user.email="ci-build@microsoft.com" -c user.name="Azure Pipelines" commit -m "Publish temporary Swift Package $current_date"
...
if [ "$SKIP_SAMPLE_APP" = true ]; then
  ...
  xcodebuild -scheme NativeAuthSampleApp -configuration Release -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.6' clean build
fi